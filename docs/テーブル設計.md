...
Storage 上の置き場所は `records/{record_id}/` とする。孤児ファイルを掃除するバッチが
`shop_photos` の写真を誤って削除しないよう、パスで分離している。
...
`photo_type` は `menu`（メニュー表）/ `exterior`（店の写真）。表示先が異なる。

| 種類 | 店一覧 | 店の詳細 |
|---|---|---|
| `exterior`（店の写真） | **サムネイルとして表示** | ヘッダーに表示 |
...
Storage 上の置き場所は `shops/{shop_id}/` とする（`record_photos` とパスで分離する）。
...
| `exterior`（店の写真） | **1人1店舗につき1枚** | 表示に使うのは最新の1枚だけで、古いものは使われないため |

外観写真を撮り直した場合は、**その人自身の前の写真と差し替える**（アップロード時に
自分の `exterior` があれば削除する）。他人の写真は削除しない。
店一覧のサムネイルには、`taken_on` が最新の `exterior` を使う。

DB 側でも重複を防ぐ。MySQL は条件付きの一意制約を書けないため、生成カラムを使う。

```sql
exterior_key VARCHAR(80) AS (
  IF(photo_type = 'exterior', CONCAT(shop_id, ':', user_id), NULL)
) STORED,
UNIQUE KEY uk_exterior (exterior_key)
```

`menu` の行では `exterior_key` が NULL になり、一意制約の対象外となる。
