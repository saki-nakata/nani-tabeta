...
class ShopControllerTest {
  ...
  void 店の取得_店がJSONで返ること() throws Exception {
    ...
    mockMvc.perform(get("/api/shops/1").with(registeredUser())).andExpect(status().isOk())
  ...
  void 店の取得_見つからない場合は404が返ること() throws Exception {
    ...
    mockMvc.perform(get("/api/shops/999").with(registeredUser())).andExpect(status().isNotFound())
  ...
  void 店の登録_新しく登録した場合は201が返ること() throws Exception {
    ...
    mockMvc.perform(post("/api/shops").with(registeredUser())
  ...
  void 店の候補の取得_店の一覧がJSONで返ること() throws Exception {
    ...
    mockMvc.perform(get("/api/shops/suggestions").with(registeredUser())
        .param("areaId", "20").param("keyword", "スター"))
  ...
  void 店の更新_更新後の店がJSONで返ること() throws Exception {
    ...
    mockMvc.perform(put("/api/shops/1").with(registeredUser())
        .contentType(MediaType.APPLICATION_JSON).content("""
  ...
  void 店の更新_見つからない場合は404が返ること() throws Exception {
    ...
    mockMvc.perform(put("/api/shops/999").with(registeredUser())
        .contentType(MediaType.APPLICATION_JSON).content("""
        {"shopName": "スタバ", "areaId": 13, "shopKindId": 1, "isClosed": true}
  ...
  void 店の取得_IDが数値でない場合は400が返ること() throws Exception {
    mockMvc.perform(get("/api/shops/abc").with(registeredUser())).andExpect(status().isBadRequest())
        .andExpect(content().json("""
            {"statusValue": 400, "statusName": "BAD_REQUEST",
             "message": "リクエストのパラメータが正しくありません。"}
            """));
  ...
  void 店の候補の取得_エリアが存在しない場合は400が返ること() throws Exception {
    ...
    mockMvc.perform(get("/api/shops/suggestions").with(registeredUser()).param("areaId", "999"))
        .andExpect(status().isBadRequest()).andExpect(content().json("""
            {"statusValue": 400, "statusName": "BAD_REQUEST",
